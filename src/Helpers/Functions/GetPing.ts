...
export const getDiscordAPIPing = (Ashen: Ashen) => {
    const AshenPing = Ashen.ws.ping;
    return {
        ping: ClampNumbers(AshenPing),
        emoji: getEmojiByPing(AshenPing),
        color: getColorByPing(AshenPing),
        message:
            AshenPing >= 0 && AshenPing <= 60
                ? "¡Ping de respuesta excelente!"
                : "¡Ping de respuesta malo!"
    };
};
